...
def query():
    query = "SELECT * FROM links WHERE ID=2 AND votes > 23"
    query = "SELECT * FROM links WHERE ID=2 OR votes > 23"
    query = "SELECT * FROM links WHERE ID=2 OR votes > 23 ORDER BY votes ASC"
    query = "SELECT * FROM links WHERE ID=2 OR votes > 23 ORDER BY votes DESC"
    query = "SELECT * FROM links ORDER BY votes DESC"
    query = "SELECT * from links where submitter_id=62443 ORDER BY submitted_time"
    # c = db.execute(query)
    # # in python, * trasforma una tuple in un oggetto
    ...
    # in python, * trasforma una tuple in un oggetto
    for link_tuple in c:
        link_object = Link(*link_tuple)
        out.append(link_object.id)
    return out
...
def link_by_id(link_id):
    #return Link(*links[link_id]).id
    for l in links:
        if l.id == link_id:
            return l

print link_by_id(2).url        
print "*****"
#print Link(*links[0]).id
...
def link_by_id2(link_id):
    # usare get con gli oggetti, controlla se l'indice esiste
